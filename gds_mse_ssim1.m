...
im = double(imread('results/samp5.tif'));
...
for i=1:Iter1
   ...
   dssim = derivative_ssim1(best_im, im);
   ...
   ssim = ssim_index(best_im, im, [0.01 0.03], ones(8));
...
for i=1:Iter2
   ...
   dssim = derivative_ssim1(worst_im, im);
   len_dmse = sum(dmse.*dmse);
   lamda = max(25, 100 - 0.1*i);	%lamda = 10;
   ...
   ssim = ssim_index(worst_im, im, [0.01 0.03], ones(8));
   [i/10000 mse/10000 ssim]
end

mse1 = mean2((best_im - im).^2);
ssim1 = ssim_index(best_im, im, [0.01 0.03], ones(8));
...
[ssim omap] = ssim_index(im, im, [0.01 0.03], ones(8));
[ssim imap] = ssim_index(init_im, im, [0.01 0.03], ones(8));
[ssim bmap] = ssim_index(best_im, im, [0.01 0.03], ones(8));
[ssim wmap] = ssim_index(worst_im, im, [0.01 0.03], ones(8));
figure(13);
...
for i=1:Iter3
   ...
   ssim_b = ssim_index(best_im, im, [0.01 0.03], ones(8));
   dssim = derivative_ssim1(best_im, im);
   tmp_im = best_im + lamda2*dssim;
   ssim_t = ssim_index(tmp_im, im, [0.01 0.03], ones(8));
...
for i=1:Iter4
   ...
   ssim_w = ssim_index(worst_im, im, [0.01 0.03], ones(8));
   dssim = derivative_ssim1(worst_im, im);
...
save('mssim_samp5_L256.mat', 'im_init', 'im_fixmse_maxssim', 'im_fixmse_minssim', 'im_fixssim_minmse', 'im_fixssim_maxmse', 'maxssim', 'minssim', 'minmse', 'maxmse', 'FIX_MSE', 'FIX_SSIM');